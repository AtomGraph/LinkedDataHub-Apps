...
await runScene({
  ...
  async body({ page, cursor, type, marks }) {
    ...
    const p1 = await addProse(page, cursor, type,
      `Order ${ORDER} shipped after its required date. Opened as a graph it shows everyone on it: the customer, the rep who booked it, the shipper who carried it. The order book charts late shipments by carrier. This page asks whether they follow the customer instead.`);
    ...
    const p2 = await addProse(page, cursor, type,
      `${CUST}: ${pick.orders} orders, ${pick.late} of them late — one in ${Math.round(pick.orders / pick.late)}. The order book already charts which carrier is late; this page charts which customer is.`);
